...
def nicinfo():
    ...
    return {'nic':nic_list}
def raminfo():
    raw_data = subprocess.check_output(["dmidecode" ,"-t", "17"])
    ...
    return ram_data
def osinfo():
    distributor = subprocess.check_output(" lsb_release -a|grep 'Distributor ID'",shell=True).decode().split(":")
...
def cpuinfo():
    ...
    data = {
        "cpu_count" : raw_data["cpu_count"].decode(),
        "cpu_core_count": raw_data["cpu_core_count"].decode()
        }
