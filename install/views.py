...
def salt_key(request):
    ...
    return render(request,'install/salt_key.html',{'saltlist_objs':saltlist_objs})
@login_required
    ...
    if request.method  == 'POST':
        ...
        if arg in ['rm -rf','rm']:
            ...
            objlogs = OperationLog.objects.create(name='命令执行',
                                                  ...
                                                  user_id=request.user.userprofile.id)
    return render_to_response('install/batchcmd.html',
           {'ret':ret},)

@login_required
def batchfile(request):
    return render(request,'install/batchfile.html')
@login_required
def remote(request):
    if request.method  == 'POST':
        print(request.POST)
    ...
    return render(request,'install/remote.html')
@login_required
def update(request):
